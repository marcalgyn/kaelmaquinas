                ...
                Experiencia.findOne({}).then((experiencia) => {
                    Rodape.findOne({}).then((rodape) => {
                        Imagem.find({}).then((imagem) => {
                            res.render("home/home", { hometopo: hometopo, servico: servico, video: video, experiencia: experiencia, rodape: rodape, imagem: imagem })
                        }).catch((erro) => {
                            res.send("Nenhuma Imagem encontrado!!")
                        })
                    }).catch((erro) => {
                        res.send("Nenhum Rodapé encontrado!!")
                    })
                    ...
                    if (!experiencia) {
                        new Experiencia({
                            ...
                            console.log("Erro ao cadastrar Experiencia Cadastrado.")
                        })
                    }
                }).catch((erro) => {
                    res.send("Nenhuma Experiencia encontrada!!")
                })

                //Insere o Primeiro Video
                if (!video) {
                    new Video({
                        titulo: "Vídeo",
                        ...
                        console.log("Erro ao cadastrar Video Cadastrado.")
                    })
                }


            }).catch((erro) => {
                res.send("Nenhum Video encontrado!!")
            })

            //Insere o Primeiro Serviço
            if (!servico) {
                new Servico({
                    titulo: "SERVIÇOS",
                    iconeservum: "fas fa-truck-moving",
                    tituloservum: "Serviço Um",
                    descservum: "descricao um serviço",

                    iconeservdois: "fas fa-truck-loading",
                    tituloservdois: "Serviço Dois",
                    descservdois: "descricao dois serviço",

                    iconeservtres: "fas fa-boxes",
                    tituloservtres: "Serviço tres",
                    descservtres: "descricao treis serviço"
                }).save().then(() => {
                    console.log("Serviço Cadastrado com sucesso")
                }).catch((erro) => {
                    console.log("Erro ao cadastrar Servio.")
                })
            }
...
router.post("/home/update-home-logo", eAdmin, upload.single("file"), (req, res, next) => {

    //const file = req.file
    var newpath = "./public/images/logo.jpg";
    var oldpath = req.file.path;
    //var file = path.join('' , newpath, 'logo.jpg');
    
    console.log("arquivo atual " + oldpath);
    fs.rename(oldpath, newpath, function (err) {
        if (err) {
            console.log("erro");
            req.flash("error_msg", "Selecione uma imagem Jpeg")
            res.redirect("/home/edit-home-logo/")
        } else {
            req.flash("success_msg", "Upload da logo realizado com sucesso")
            res.redirect("/vis-home")
        }
...
router.post("desativado/home/update-home-logo", eAdmin, uploads.single("file"), (req, res, next) => {
    
    console.log("Acesso 0 " + Object.values(req) );
